...
import com.wl.wlmall.utils.LayoutUtils;
...
public class LoadingLayout extends FrameLayout {
    ...
    public final static int Sub_loading = 5;
    private int state;

    private Context mContext;
    private View loadingPage;
    private View subLoadingPage;
    ...
    public LoadingLayout(Context context, AttributeSet attrs) {
        ...
        pageBackground = a.getColor(R.styleable.LoadingLayout_pageBackground, LayoutUtils.getColor(mContext, R.color
    ...
    private void build() {
        ...
        if (mConfig.subLoadingView == null) {
            subLoadingPage = LayoutInflater.from(mContext).inflate(mConfig.subLoadingLayoutId, null);
            subLoadingPage.findViewById(R.id.sub_bg_layout).setOnClickListener(new OnClickListener() {
                @Override
            ...
            subLoadingPage = mConfig.subLoadingView;
        }
        ...
        subLoadingPage.setBackgroundColor(ContextCompat.getColor(mContext,R.color.alpha_a_black));
        ...
        errorText = LayoutUtils.findViewById(errorPage, R.id.error_text);
        emptyText = LayoutUtils.findViewById(emptyPage, R.id.empty_text);
        networkText = LayoutUtils.findViewById(networkPage, R.id.no_network_text);

        errorImg = LayoutUtils.findViewById(errorPage, R.id.error_img);
        emptyImg = LayoutUtils.findViewById(emptyPage, R.id.empty_img);
        networkImg = LayoutUtils.findViewById(networkPage, R.id.no_network_img);

        errorReloadBtn = LayoutUtils.findViewById(errorPage, R.id.error_reload_btn);
        networkReloadBtn = LayoutUtils.findViewById(networkPage, R.id.no_network_reload_btn);
        ...
        errorText.setTextColor(LayoutUtils.getColor(mContext, mConfig.tipTextColor));
        emptyText.setTextColor(LayoutUtils.getColor(mContext, mConfig.tipTextColor));
        networkText.setTextColor(LayoutUtils.getColor(mContext, mConfig.tipTextColor));
        ...
        errorReloadBtn.setTextColor(LayoutUtils.getColor(mContext, mConfig.buttonTextColor));
        networkReloadBtn.setTextColor(LayoutUtils.getColor(mContext, mConfig.buttonTextColor));

        if (mConfig.buttonHeight != -1) {

            errorReloadBtn.setHeight(LayoutUtils.dp2px(mContext, mConfig.buttonHeight));
            networkReloadBtn.setHeight(LayoutUtils.dp2px(mContext, mConfig.buttonHeight));
        }
        if (mConfig.buttonWidth != -1) {

            errorReloadBtn.setWidth(LayoutUtils.dp2px(mContext, mConfig.buttonWidth));
            networkReloadBtn.setWidth(LayoutUtils.dp2px(mContext, mConfig.buttonWidth));
        }
        ...
        switch (status) {
            case Success:
                ...
                if (defineLoadingPage != null) {
                    ...
                    subLoadingPage.setVisibility(GONE);
                }
                break;
            ...
            case Sub_loading:
                ...
                if (defineLoadingPage != null) {
                    ...
                    subLoadingPage.setVisibility(VISIBLE);
                }
                break;
    ...
    public LoadingLayout setReloadButtonTextColor(@ColorRes int id) {
        ...
        errorReloadBtn.setTextColor(LayoutUtils.getColor(mContext, id));
        networkReloadBtn.setTextSize(LayoutUtils.getColor(mContext, id));
        return this;
    }
    ...
    public LoadingLayout setDefineBackgroundColor(@ColorRes int color) {
        ...
        if (defineLoadingPage == null) {
            loadingPage.setBackgroundColor(LayoutUtils.getColor(mContext, color));
        } else {
            defineLoadingPage.setBackgroundColor(LayoutUtils.getColor(mContext, color));
        }
        errorPage.setBackgroundColor(LayoutUtils.getColor(mContext, color));
        emptyPage.setBackgroundColor(LayoutUtils.getColor(mContext, color));
        networkPage.setBackgroundColor(LayoutUtils.getColor(mContext, color));
        return this;
    }
    ...
    @IntDef({Success, Empty, Error, No_Network, Loading,Sub_loading})
    ...
    public static class Config {
        ...
        String errorStr = "哎呀，数据消失了";
        String netwrokStr = "无网络连接，请检查网络···";
        String reloadBtnStr = "点击重试";
        int emptyImgId = R.mipmap.empty;
        int errorImgId = R.mipmap.pig_no_data;
        int networkImgId = R.mipmap.pig_no_network;
        ...
        int subLoadingLayoutId = R.layout.widget_sub_loading_page;
        View loadingView = null;
        View subLoadingView = null;
